...
local function InitDatabase(callback)
    MySQL.Async.execute([[
        ...
        print('[FIRE] Datenbank-Tabelle bereit')
        if callback then
            callback()
        end
    end)
...
function LoadFireLocationsFromDB(callback)
    MySQL.Async.fetchAll('SELECT * FROM fire_locations', {}, function(result)
        if result and #result > 0 then
            ...
            print('[FIRE] ' .. #result .. ' Locations aus Datenbank geladen')
        else
            print('[FIRE] Keine Locations in Datenbank gefunden')
        end
...
QBCore.Commands.Add('firecreator', 'Fire Creator Panel öffnen (Admin)', {}, false, function(source)
    print('^1[DPS-FIRE DEBUG]^7 Command /firecreator wurde von Spieler ' .. source .. ' ausgeführt')
    ...
    if not hasPermission then
        TriggerClientEvent('QBCore:Notify', source, 'Keine Berechtigung! Benötigt: Admin-Rechte', 'error')
        print("^3[DPS-FIRE]^7 " .. GetPlayerName(source) .. " (ID: " .. source .. ") hat keine Admin-Berechtigung für /firecreator")
        return
    end
    
    print("^2[DPS-FIRE]^7 " .. GetPlayerName(source) .. " hat Fire Creator UI geöffnet")
...
RegisterNetEvent('qb-fire:addLocation', function(coords)
    ...
    MySQL.Async.execute('INSERT INTO fire_locations (x, y, z) VALUES (@x, @y, @z)', {
        ...
        if rowsChanged > 0 then
            ...
            TriggerClientEvent('QBCore:Notify', source, 'Spawn-Punkt hinzugefügt!', 'success')
            
            -- Sende Update an Client der die Location hinzugefügt hat
            local locations = {}
            for _, loc in ipairs(Config.FireLocations) do
                table.insert(locations, {x = loc.x, y = loc.y, z = loc.z})
            end
    ...
    if Config.FireLocations[index + 1] then
        ...
        MySQL.Async.execute('DELETE FROM fire_locations WHERE x = @x AND y = @y AND z = @z LIMIT 1', {
            ...
            if rowsChanged > 0 then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Spawn-Punkt gelöscht!', 'success')
                
                -- Sende aktualisierte Liste an Client
                local locations = {}
                for _, loc in ipairs(Config.FireLocations) do
                    table.insert(locations, {x = loc.x, y = loc.y, z = loc.z})
                end
...
local function SyncConfigLocationsToDatabase(callback)
    print('[FIRE] Synchronisiere Config-Locations zur Datenbank...')
    local totalLocations = #Config.FireLocations
    local checkedCount = 0
    local addedCount = 0
    
    -- Erst alle bestehenden aus Config laden (als Backup)
    local configBackup = {}
    for _, coords in ipairs(Config.FireLocations) do
        table.insert(configBackup, coords)
    end
    
    if totalLocations == 0 then
        print('[FIRE] Keine Config-Locations zum Synchronisieren gefunden')
        if callback then callback() end
        return
    end
    
    for _, coords in ipairs(configBackup) do
        MySQL.Async.fetchScalar('SELECT COUNT(*) FROM fire_locations WHERE x = @x AND y = @y AND z = @z', {
            ['@x'] = coords.x,
            ['@y'] = coords.y,
            ['@z'] = coords.z
        }, function(count)
            if count == 0 then
                MySQL.Async.execute('INSERT INTO fire_locations (x, y, z) VALUES (@x, @y, @z)', {
                    ...
                    if rowsChanged > 0 then
                        ...
                        print('[FIRE] Location hinzugefügt: ' .. coords.x .. ', ' .. coords.y .. ', ' .. coords.z)
                    end
                    
                    checkedCount = checkedCount + 1
                    if checkedCount >= totalLocations then
                        print('[FIRE] Sync abgeschlossen: ' .. addedCount .. ' neue Locations hinzugefügt')
                        Wait(500)
                        LoadFireLocationsFromDB(callback)
                    end
                end)
            else
                checkedCount = checkedCount + 1
...
CreateThread(function()
    ...
    InitDatabase(function()
        ...
        if Config.AutoSyncLocations then
            print('[FIRE] AutoSyncLocations ist aktiviert')
            SyncConfigLocationsToDatabase(function()
                print('[FIRE] System bereit mit ' .. #Config.FireLocations .. ' Locations')
            end)
        else
            print('[FIRE] AutoSyncLocations ist deaktiviert')
            LoadFireLocationsFromDB(function()
                print('[FIRE] System bereit mit ' .. #Config.FireLocations .. ' Locations')
            end)
        end
    end)
...
CreateThread(function()
    while true do
        ...
        if Config.AutoSpawnFires and #Config.FireLocations > 0 then
...
RegisterCommand('startfire', function(source)
    ...
    TriggerClientEvent('chat:addMessage', source, {
        color = {255, 120, 0},
        args = {'SYSTEM', '🔥 Feuer gestartet'}
    })
end, false)

RegisterCommand('syncfirelocations', function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    ...
    if IsPlayerAceAllowed(source, "command.syncfirelocations") then
        hasPermission = true
    end
    ...
    TriggerClientEvent('QBCore:Notify', source, 'Synchronisiere Locations...', 'info')
    
    -- Lade zuerst die Config neu
    local configLocations = {}
    for _, coords in ipairs(Config.FireLocations) do
        table.insert(configLocations, coords)
    end
    
    print('[FIRE] Starte manuelle Synchronisation mit ' .. #configLocations .. ' Config-Locations')
    
    SyncConfigLocationsToDatabase(function()
        TriggerClientEvent('QBCore:Notify', source, #Config.FireLocations .. ' Locations synchronisiert!', 'success')
        print('[FIRE] Manuelle Synchronisation abgeschlossen')
    end)
end, false)
